...
*/
	...
	IEnumerator Shake(float amount, float duration) {
		...
		while (duration > 0.01f) {
			...
			if(smooth)
				camera.localRotation = Quaternion.Lerp(camera.localRotation, Quaternion.Euler(rotationAmount), Time.deltaTime * smoothAmount);
			else
				camera.localRotation = Quaternion.Euler (rotationAmount);//Set the local rotation the be the rotation amount.
